...
(function(){
  ...
  window.saveListingEdit=async function(id){
    ...
    if(error)return toast(error.message);closeModal();toast('E’lon yangilandi');route();
  };
  window.deactivateListing=async function(id){
    if(!state.user)return auth();if(!confirm('Bu e’lonni faol ro‘yxatdan chiqaramizmi?'))return;
    ...
    if(error)return toast(error.message);toast('E’lon faol ro‘yxatdan chiqarildi');route();
  };
  window.reactivateListing=async function(id){
    ...
    if(error)return toast(error.message);toast('E’lon qayta moderatsiyaga yuborildi');route();
  };
  window.editProfile=async function(){
    if(!state.user)return auth();const p=state.profile||{};
  ...
  window.saveProfileEdit=async function(){
    if(!state.user)return auth();const name=s(document.getElementById('pn')?.value),phone=s(document.getElementById('pp')?.value),avatar_url=s(document.getElementById('pa')?.value);
    if(name.length<2)return toast('Ismni kiriting');if(phone&&!/^[+()\-\s\d]{7,30}$/.test(phone))return toast('Telefon raqamini tekshiring');
    ...
    if(error)return toast(error.message);state.profile=data||{...state.profile,name,phone,avatar_url};closeModal();toast('Profil saqlandi');route();
  };
  window.openAccountSettings=function(){
    if(!state.user)return auth();openModal(`<h2>Hisob sozlamalari</h2><div class="muted">${e(state.user.email||'')}</div><input id="ne" class="input" style="margin-top:10px" type="email" placeholder="Yangi email"><div class="actions" style="margin-top:14px"><button class="btn primary" onclick="changeAuthEmail()">Emailni o‘zgartirish</button><button class="btn" onclick="closeModal()">Yopish</button></div>`);
  };
  window.changeAuthEmail=async function(){
    if(!state.user)return auth();const next=s(document.getElementById('ne')?.value);if(!/^\S+@\S+\.\S+$/.test(next))return toast('Emailni to‘g‘ri kiriting');
    const {error}=await sb.auth.updateUser({email:next});if(error)return toast(error.message);closeModal();toast('Tasdiqlash havolasi yangi emailga yuborildi');
  };
