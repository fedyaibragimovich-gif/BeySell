...
(function(){
  ...
  window.confirmSaveSearch=async function(){
    if(!state.user)return auth();
    const name=val(document.getElementById('ssName')?.value);
    if(name.length<2)return toast('Qidiruv nomini kiriting');
    ...
    if(error)return toast('Saqlashda xatolik: '+error.message);
    closeModal();toast('Qidiruv saqlandi');
  };
  ...
  window.deleteSavedSearch=async function(id){
    if(!state.user)return auth();
    const {error}=await sb.from('saved_searches').delete().eq('id',id).eq('user_id',state.user.id);
  ...
  if(typeof originalSearchPage==='function'){
    window.searchPage=async function(){
      ...
      const add=`<div style="margin:0 0 14px"><button class="btn" onclick="saveCurrentSearch()">♡ Qidiruvni saqlash</button> <button class="btn" onclick="go('#saved-searches')">Saqlangan qidiruvlar</button></div>`;
      return add+html;
    };
  }
  ...
  if(typeof originalRoute==='function'){
    window.route=async function(){
      ...
      if(h==='#saved-searches'){
        const body=await savedSearches();
        renderSavedSearchesPage(body);
        return;
      }
